...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "About to spawn LIVEVIDEO process from pid %d\n", getpid());
    pid_t p;
    int r = posix_spawn(&p, "./LIVEVIDEO", nullptr, nullptr,
                       (char**) args, nullptr);
    //assert(r == 0);
    std::cout << r << std::endl;

    fprintf(stderr, "Child pid %d should run my LIVEVIDEO\n", p);
return 0;
}
